...
| | Status |
|:---|:---|
| Build | [![ CI ](https://github.com/${env:GITHUB_REPOSITORY_OWNER}/$($project.repository)/actions/workflows/ci-windows.yml/badge.svg)](https://github.com/${env:GITHUB_REPOSITORY_OWNER}/$($project.repository)/actions/workflows/ci-windows.yml) |
| Unit Tests | [![ Unit Tests ](https://github.com/${env:GITHUB_REPOSITORY_OWNER}/$($project.repository)/actions/workflows/unit-tests-windows.yml/badge.svg)](https://github.com/${env:GITHUB_REPOSITORY_OWNER}/$($project.repository)/actions/workflows/unit-tests-windows.yml)|
...
function Update-BranchProtection {
  ...
  gh api `
    ...
    /repos/${env:GITHUB_REPOSITORY_OWNER}/$($project.repository)/branches/main/protection `
      ...
      -F "required_status_checks[checks][][context]=validate-jira-issues" `
      ...
      -f "bypass_pull_request_allowances[teams[]]=digital-is-superuser" `
      -F "restrictions=null" `
...
function Set-RepositoryTeam {
  ...
  gh api `
    ...
    -H "Accept: application/vnd.github+json" `
    -H "X-GitHub-Api-Version: 2022-11-28" `
    /orgs/${env:GITHUB_REPOSITORY_OWNER}/teams/$($teamName -replace "${env:GITHUB_REPOSITORY_OWNER}/",'')/repos/${env:GITHUB_REPOSITORY_OWNER}/$($project.repository) `
...
try {
  ...
  gh repo create ${env:GITHUB_REPOSITORY_OWNER}/$($project.repository) --internal --template $template --clone --description $DESCRIPTION
